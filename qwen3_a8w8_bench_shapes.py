...
parser = argparse.ArgumentParser(description="Benchmark GEMM shapes from unique dims JSON")
parser.add_argument("--dims_json", type=str, default='unique_dims.json', help="Path to the JSON file with unique dims")
...
try:
    ...
    for m, n, k in unique_shapes:
        res = []
        for metric in ["throughput", "time", "bandwidth"]:
            cmd = "TRITON_HIP_ASYNC_COPY_BYPASS_PERMUTE=1" \
                " TRITON_HIP_USE_ASYNC_COPY=1" \
                " AMDGCN_USE_BUFFER_OPS=1" \
                " TRITON_HIP_USE_BLOCK_PINGPONG=1 "
            cmd += f"python op_tests/op_benchmarks/triton/bench_gemm_a8w8_blockscale.py --shape {m} {n} {k} --metric {metric}"
            print(f"Running: {cmd}")
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
            val = float(result.stdout.split()[-1])
            res.append(val)
        res_bench.append((m, n, k, *res))
        print(f"Result for M={m}, N={n}, K={k}: {res}")
    
    # Create DataFrame
    bench_df = pd.DataFrame(res_bench, columns=['M', 'N', 'K', 'throughput (TFLOPs)', 'time (ms)', 'bandwidth (GB/s)'])
